...
const UNLINKED_ATTENDANCE_INCIDENT_START_DATE_ENV =
  "VISO_ATTENDANCE_UNLINKED_INCIDENT_START_DATE";
const UNLINKED_ATTENDANCE_INCIDENT_LOOKBACK_DAYS_ENV =
...
function normalizeTimeZone(rawValue: string | null): string {
  ...
  try {
    new Intl.DateTimeFormat("es-CO", { timeZone: candidate }).format(
      new Date(),
    );
    return candidate;
  } catch {
...
function getOptionalEnvDateMs(value: string | undefined): number | null {
  ...
  const parsed = new Date(
    candidate.includes("T") ? candidate : `${candidate}T00:00:00-05:00`,
  );
...
function getUnlinkedAttendanceIncidentCutoffMs(reportEnd: Date): number | null {
  const configuredStartMs = getOptionalEnvDateMs(
    process.env[UNLINKED_ATTENDANCE_INCIDENT_START_DATE_ENV],
  );
...
function isRestShiftLike(
  shift: Pick<ScheduledShiftRow, "status" | "notes">,
): boolean {
...
function buildAttendanceSessions(
  ...
  const breaksByEmployeeSite = new Map<
    string,
  ...
  for (const [employeeId, rows] of logsByEmployee.entries()) {
    ...
    const closeSession = (
      ...
      if (departure?.occurred_at) {
        const distanceLabel =
          departure.distance_meters != null
            ? ` (${Math.round(departure.distance_meters)}m)`
            : "";
        observations.push(
      ...
      if (isAutoClose && checkOutRow?.occurred_at) {
        observations.push(
          `Cierre automático ${formatTime(checkOutRow.occurred_at, timeZone)}`,
        );
      }
      if (status === "Abierto") {
  ...
  const rows = [...scheduledShifts]
    .sort((a, b) => {
      ...
      if (employeeA !== employeeB)
        return employeeA.localeCompare(employeeB, "es");
    ...
    .map((shift) => {
      ...
      const scheduledStartAt = zonedLocalToUtc(
        shift.shift_date,
        shift.start_time,
        timeZone,
      );
      const scheduledEndAt = zonedLocalToUtc(
        shift.shift_date,
        shift.end_time,
        timeZone,
      );
      ...
      const scheduledGrossMinutes = safeMinutes(
        (scheduledEndMs - scheduledStartMs) / 60000,
      );
      const scheduledBreakMinutes = isRestDay
        ? 0
        : safeMinutes(shift.break_minutes ?? 0);
      const scheduledNetMinutes = isRestDay
        ? 0
        : safeMinutes(scheduledGrossMinutes - scheduledBreakMinutes);
      const employeeSessions = sessionsByEmployee.get(shift.employee_id) ?? [];

      let matched: AttendanceSession | null =
        employeeSessions.find(
          (session) =>
            !usedSessionKeys.has(session.key) && session.shiftId === shift.id,
        ) ?? null;
      let matchedBy: "shift_id" | "window" | "none" = matched
        ? "shift_id"
        : "none";
        ...
        const candidates = employeeSessions
          ...
          .sort((left, right) => {
            const leftDiff = Math.abs(
              new Date(left.checkInAt).getTime() - scheduledStartMs,
            );
            const rightDiff = Math.abs(
              new Date(right.checkInAt).getTime() - scheduledStartMs,
            );
      ...
      const effectiveEndMs = matched
        ? new Date(matched.effectiveEndAt).getTime()
        : null;
      const lateMinutes =
        !isRestDay && checkInMs != null
          ? safeMinutes((checkInMs - scheduledStartMs) / 60000)
          : 0;
      const isLate =
        !isRestDay &&
        checkInMs != null &&
        checkInMs > scheduledStartMs + lateGraceMinutes * 60000;
      const overtimeMinutes =
        !isRestDay && effectiveEndMs != null
          ? safeMinutes((effectiveEndMs - scheduledEndMs) / 60000)
          : 0;
      const leftEarlyMinutes =
        !isRestDay && matched?.checkOutAt != null
          ? safeMinutes(
              (scheduledEndMs - new Date(matched.checkOutAt).getTime()) / 60000,
            )
          : 0;
      const isNoShow = !isRestDay && !matched && shiftEnded;
      const isOpen = !isRestDay && matched?.status === "Abierto";
      const missingCloseCountable =
        !isRestDay && !!matched && isOpen && shiftEnded;
...
function buildEmployeeSummary(
  ...
  const byEmployee = new Map<
    string,
    EmployeeSummary & { _siteSet: Set<string> }
  >();
  ...
  for (const row of rows) {
    ...
    if (row.isOpen && row.attendanceStatus === "Abierto")
      current.missingCloseCount += 1;
...
function buildReportSummary(rows: ConsolidatedShiftRecord[]): ReportSummary {
  ...
  const missingCloseCount = countableRows.filter(
    (row) => row.isOpen && row.attendanceStatus === "Abierto",
  ).length;
  const autoCloseCount = countableRows.filter((row) => row.isAutoClose).length;
  const departureCount = countableRows.filter(
    (row) => row.hasDepartureEvent,
  ).length;
  const scheduledMinutes = countableRows.reduce(
    (sum, row) => sum + row.scheduledNetMinutes,
    0,
  );
  const netMinutes = countableRows.reduce(
    (sum, row) => sum + row.actualNetMinutes,
    0,
  );
  const attendanceRate =
    scheduledShifts > 0 ? attendedShifts / scheduledShifts : 0;
  const punctualityRate =
    attendedShifts > 0 ? (attendedShifts - lateCount) / attendedShifts : 0;
...
function buildIncidentRows(
  ...
  for (const row of rows) {
    ...
    if (row.hasDepartureEvent) {
      const distanceLabel =
        row.departureDistanceMeters != null
          ? ` Distancia: ${Math.round(row.departureDistanceMeters)}m.`
          : "";
      incidents.push({
    ...
    if (row.matchedBy === "window") {
      incidents.push({
        ...
        status: row.attendanceStatus,
        detail:
          "La asistencia se vinculó por ventana horaria y no por shift_id.",
      });
    }
  }
  ...
  for (const session of sessions) {
    ...
    incidents.push({
      ...
      detail:
        "Existe asistencia reciente registrada que no pudo vincularse a un turno publicado del rango.",
    });
  }
...
function buildWorkbook(
  ...
  summarySheet.getCell("A2").value =
    `Periodo: ${formatDate(start, timeZone)} a ${formatDate(end, timeZone)} | Alcance: ${scopeLabel} | Generado: ${formatDate(new Date(), timeZone)}`;
  ...
  if (employeeSummaryRows.length === 0) {
    ...
    summarySheet.getCell(`A${employeeRow}`).value =
      "Sin turnos programados para el rango seleccionado.";
    summarySheet.getCell(`A${employeeRow}`).font = { size: 9, italic: true };
    summarySheet.getCell(`A${employeeRow}`).alignment = {
      vertical: "middle",
      horizontal: "center",
    };
    employeeRow += 1;
  ...
  detailSheet.getCell("A2").value =
    `Periodo: ${formatDate(start, timeZone)} a ${formatDate(end, timeZone)} | Alcance: ${scopeLabel}`;
  ...
  for (const [index, row] of rows.entries()) {
    detailSheet.getRow(detailRow).values = [
      ...
      row.checkOutAt
        ? formatDateTime(row.checkOutAt, timeZone)
        : row.isOpen
          ? "Abierto"
          : "-",
  ...
  if (rows.length === 0) {
    ...
    detailSheet.getCell(`A${detailRow}`).value =
  ...
  incidentsSheet.getCell("A1").alignment = {
    vertical: "middle",
    horizontal: "center",
  };
  ...
  if (incidentRows.length === 0) {
    ...
    incidentsSheet.getCell("A5").value =
      "Sin incidencias operativas en el rango seleccionado.";
    incidentsSheet.getCell("A5").font = { size: 9, italic: true };
    incidentsSheet.getCell("A5").alignment = {
      vertical: "middle",
      horizontal: "center",
    };
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const requestedEmployeeId =
      url.searchParams.get("employee_id")?.trim() || null;
    const requestedSiteId = url.searchParams.get("site_id")?.trim() || null;
    const responseFormat = (url.searchParams.get("format") ?? "xlsx")
      .trim()
      .toLowerCase();
    ...
    if (isGlobalRole) {
      ...
      if (scopeSiteId) {
        ...
        if (scopedSiteError || !scopedSite) {
          return new Response(
            JSON.stringify({ error: "Site scope not found" }),
            {
              status: 404,
              headers: jsonHeaders,
            },
    ...
    if (scopeEmployeeId) {
      const { data: scopedEmployee, error: scopedEmployeeError } =
        await supabase
          .from("employees")
      ...
      if (scopedEmployeeError || !scopedEmployee) {
        return new Response(
          JSON.stringify({ error: "Employee scope not found" }),
          {
            status: 404,
            headers: jsonHeaders,
          },
      ...
      if (isGlobalRole && scopeSiteId) {
        ...
        if (!primaryMatchesSite) {
          const { data: employeeSiteMatch, error: employeeSiteMatchError } =
            await supabase
              .from("employee_sites")
              .select("employee_id")
          ...
          if (employeeSiteMatchError) {
            return new Response(
              JSON.stringify({ error: "Employee site validation failed" }),
              {
                status: 500,
                headers: jsonHeaders,
              },
        ...
        if (!primaryMatchesSite && !assignmentMatchesSite) {
          return new Response(
            JSON.stringify({
              error: "Employee does not belong to selected site",
            }),
            {
              status: 400,
              headers: jsonHeaders,
      ...
      if (isGlobalRole && scopeEmployeeId !== userId) {
        const employeeLabel =
          scopedEmployee.alias ?? scopedEmployee.full_name ?? scopedEmployee.id;
    ...
    const [
      ...
      (() => {
        let query = supabase
          ...
          .select(
            "employee_id, site_id, shift_start_at, event_type, occurred_at, distance_meters, notes",
          )
          .gte("occurred_at", queryStart.toISOString())
          .lte("occurred_at", queryEnd.toISOString())
          .order("occurred_at", { ascending: true });
    ...
    if (policyError) {
      return new Response(
        JSON.stringify({ error: "Shift policy query failed" }),
        {
          status: 500,
          headers: jsonHeaders,
        },
    ...
    if (attendanceError) {
      return new Response(
        JSON.stringify({ error: "Attendance query failed" }),
        {
          status: 500,
          headers: jsonHeaders,
        },
    ...
    if (eventError) {
      return new Response(
        JSON.stringify({ error: "Shift events query failed" }),
        {
          status: 500,
          headers: jsonHeaders,
        },
    ...
    if (shiftError) {
      return new Response(
        JSON.stringify({ error: "Shift schedule query failed" }),
        {
          status: 500,
          headers: jsonHeaders,
        },
    ...
    const lateGraceMinutes = safeMinutes(
      policy?.late_grace_minutes ?? DEFAULT_LATE_GRACE_MINUTES,
    );
    const autoCloseGraceMinutes = safeMinutes(
      policy?.auto_checkout_grace_minutes_after_end ??
    ...
    const sessions = buildAttendanceSessions(
      attendanceRows,
      breakRows,
      eventRows,
      endIso,
    ...
    const unlinkedAttendanceIncidentCutoffMs =
      getUnlinkedAttendanceIncidentCutoffMs(end);
    ...
    const incidentRows = buildIncidentRows(
      rows,
      sessions,
      consolidated.usedSessionKeys,
      reportTimeZone,
      {
    ...
    if (responseFormat === "json") {
      ...
      return new Response(
        JSON.stringify({
          ...
          incidentPolicy: {
            unlinkedAttendanceIncidentStartAt:
              unlinkedAttendanceIncidentCutoffMs != null
                ? new Date(unlinkedAttendanceIncidentCutoffMs).toISOString()
                : null,
            unlinkedAttendanceIncidentLookbackDays: process.env[
              UNLINKED_ATTENDANCE_INCIDENT_START_DATE_ENV
            ]
              ? null
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "No se pudo procesar el reporte de asistencia.",
      },
      { status: 500 },
    );
  }
