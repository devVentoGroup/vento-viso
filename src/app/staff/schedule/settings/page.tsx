...
async function saveCoverageRequirementAction(formData: FormData) {
  ...
  const returnTo = asText(formData.get("return_to")) || "/staff/schedule/settings";
  ...
  const { error } = await supabase.schema("viso").from("site_staffing_requirements").insert({
    site_id: siteId,
...
async function deleteCoverageRequirementAction(formData: FormData) {
  ...
  if (!id) redirect(`${returnTo}&error=${encodeURIComponent("Regla de cobertura inválida.")}`);

  const supabase = createAdminClient();
  const { error } = await supabase.schema("viso").from("site_staffing_requirements").delete().eq("id", id);
  if (error) redirect(`${returnTo}&error=${encodeURIComponent(error.message)}`);
...
async function saveAvailabilityAction(formData: FormData) {
  ...
  const availabilityKind = asText(formData.get("availability_kind")) as AvailabilityRow["availability_kind"];
  ...
  const supabase = createAdminClient();
  const { error } = await supabase.schema("viso").from("employee_availability").insert({
...
async function deleteAvailabilityAction(formData: FormData) {
  ...
  if (!id) redirect(`${returnTo}&error=${encodeURIComponent("Disponibilidad inválida.")}`);

  const supabase = createAdminClient();
  const { error } = await supabase.schema("viso").from("employee_availability").delete().eq("id", id);
  if (error) redirect(`${returnTo}&error=${encodeURIComponent(error.message)}`);
...
async function saveWorkerRulesAction(formData: FormData) {
  ...
  const returnTo = asText(formData.get("return_to")) || "/staff/schedule/settings";
  const employeeId = asText(formData.get("employee_id"));
  const targetWeeklyMinutes = Math.max(0, asNumber(formData.get("target_weekly_minutes"), 2400));
  const maxWeeklyMinutes = Math.max(0, asNumber(formData.get("max_weekly_minutes"), 2880));
  ...
  const supabase = createAdminClient();
  const { error: limitsError } = await supabase.schema("viso").from("employee_planning_limits").upsert(
    {
      employee_id: employeeId,
      site_id: siteId,
  ...
  if (limitsError) redirect(`${returnTo}&error=${encodeURIComponent(limitsError.message)}`);

  const { error: preferencesError } = await supabase.schema("viso").from("employee_shift_preferences").upsert(
    {
      employee_id: employeeId,
      site_id: siteId,
  ...
  if (preferencesError) redirect(`${returnTo}&error=${encodeURIComponent(preferencesError.message)}`);
...
export default async function StaffScheduleSettingsPage({
  ...
  const supabase = createAdminClient();
  const { data: sitesData } = await supabase.from("sites").select("id,name,code").order("name", { ascending: true });
  const sites = (sitesData ?? []) as SiteRow[];
  const selectedSiteId = sp.site_id && sites.some((site) => site.id === sp.site_id) ? String(sp.site_id) : sites[0]?.id ?? "";
  ...
  const [
    ...
    selectedSiteId
      ? supabase.from("employees").select("id,full_name,alias,role,is_active,site_id").eq("site_id", selectedSiteId).eq("is_active", true).order("full_name", { ascending: true })
      : Promise.resolve({ data: [], error: null }),
    selectedSiteId
      ? supabase.from("employee_sites").select("employee_id,is_active,employee:employees(id,full_name,alias,role,is_active,site_id)").eq("site_id", selectedSiteId).eq("is_active", true)
      : Promise.resolve({ data: [], error: null }),
    selectedSiteId
      ? supabase.schema("viso").from("site_staffing_requirements").select("id,site_id,day_of_week,start_time,end_time,min_headcount,ideal_headcount,max_headcount,required_role_code").eq("site_id", selectedSiteId).order("day_of_week", { ascending: true }).order("start_time", { ascending: true })
      : Promise.resolve({ data: [], error: null }),
    selectedSiteId
      ? supabase.schema("viso").from("employee_availability").select("id,employee_id,site_id,day_of_week,available_from,available_to,is_available,availability_kind").eq("site_id", selectedSiteId).order("day_of_week", { ascending: true }).order("available_from", { ascending: true })
      : Promise.resolve({ data: [], error: null }),
    selectedSiteId
      ? supabase.schema("viso").from("employee_planning_limits").select("employee_id,target_weekly_minutes,max_weekly_minutes").eq("site_id", selectedSiteId)
      : Promise.resolve({ data: [], error: null }),
    selectedSiteId
      ? supabase.schema("viso").from("employee_shift_preferences").select("employee_id,prefers_morning,prefers_afternoon,prefers_evening,avoid_opening,avoid_closing").eq("site_id", selectedSiteId)
      : Promise.resolve({ data: [], error: null }),
  ]);

  const employeeMap = new Map<string, EmployeeRow>();
  for (const row of (directEmployeesRes.data ?? []) as EmployeeRow[]) employeeMap.set(row.id, row);
  ...
  const roleOptions = [...new Set(employees.map((employee) => employee.role).filter(Boolean) as string[])].sort((a, b) =>
    a.localeCompare(b, "es"),
  );
  const staffingRequirements = (staffingRequirementsRes.data ?? []) as StaffingRequirementRow[];
  const availabilityConfigRows = (availabilityConfigRes.data ?? []) as AvailabilityRow[];
  const planningLimitsRows = (planningLimitsRes.data ?? []) as Array<{ employee_id: string; target_weekly_minutes: number; max_weekly_minutes: number }>;
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            <Link href={buildPlannerHref(selectedSiteId, weekStartIso)} className="ui-btn ui-btn--ghost">
              Volver al planner
            </Link>
          </div>
        }
      ...
      {okMsg ? (
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-800">{okMsg}</div>
      ) : null}

      <div className="ui-panel space-y-4">
        <form method="get" className="grid gap-3 md:grid-cols-[minmax(260px,1fr)_180px_auto]">
          ...
          <label className="flex flex-col gap-1">
            ...
            <input name="week" type="date" className="ui-input" defaultValue={weekStartIso} />
          </label>
          <div className="flex items-end">
            <button type="submit" className="ui-btn ui-btn--ghost w-full">Actualizar</button>
          </div>
        </form>
      </div>
      ...
      {!selectedSiteId ? (
        <div className="ui-panel">
          <div className="ui-empty">No hay sedes disponibles para configurar.</div>
        </div>
      ) : employees.length === 0 ? (
        <div className="ui-panel">
          <div className="ui-empty">
            <p className="font-semibold text-[var(--ui-text)]">No hay trabajadores en esta sede.</p>
            <p className="mt-2 text-sm text-[var(--ui-muted)]">
              Primero asigna personal a la sede y luego configura reglas del planificador.
            </p>
          </div>
        </div>
      ) : (
